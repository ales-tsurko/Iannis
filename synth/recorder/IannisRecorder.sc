IannisRecorder {
    ...
    *new {arg recordingDir, delegate, outputBus;
        ^super.new.init(recordingDir, delegate, outputBus);
    }

    init {arg samplesDir, viewController, aBus;
        outputBus = aBus;
        ...
        if(isPlaying.not && isRecording.not) {
            Routine({
                ...
                playerSynth = Synth(\iannis_sample_player, [
                    ...
                    \fadeDur, fadeDur,
                    \out, outputBus
                ], delegate.synthController.node, \addBefore);
